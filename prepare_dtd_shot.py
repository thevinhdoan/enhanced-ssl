#!/usr/bin/env python3
"""
Utility to generate DTD supervised configs for a new shot count and refresh the run scripts.

Steps performed (default is generate only; add --run to execute training/eval):
1) Clone the four DTD supervised template configs to the requested shot, scaling
...
TEMPLATES: List[Tuple[str, str, Path]] = [
    ("adaptformer_clip", "clip", Path("config/adaptformer/supervised/dtd/3-shot/clip/dpr-0/train-aug-strong/{'adapter_scaler': 0.1, 'adapter_bottleneck': 16}/lr-1e-3/config.yaml")),
    ("lora_clip", "clip", Path("config/lora/supervised/dtd/3-shot/clip/dpr-0/train-aug-strong/{'lora_bottleneck': 16}/lr-1e-3/config.yaml")),
    ("adaptformer_dinov2", "dinov2", Path("config/adaptformer/supervised/dtd/3-shot/dinov2/dpr-0.2/train-aug-strong/{'adapter_scaler': 0.1, 'adapter_bottleneck': 4}/lr-1e-3/config.yaml")),
    ("lora_dinov2", "dinov2", Path("config/lora/supervised/dtd/3-shot/dinov2/dpr-0/train-aug-strong/{'lora_bottleneck': 16}/lr-1e-3/config.yaml")),
]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Prepare DTD configs and run scripts for a new shot count.")
...
def rewrite_run_script(path: Path, new_commands: List[str], shot: int) -> None:
    ...
    banner = f"# Auto-generated for DTD {shot}-shot; previous lines preserved above."
