...
class BlockPETL(nn.Module):
    def __init__(
        ...
        if peft_config.ft_attn_module:
            self.ft_attn_module = MODULE_REGISTRY[peft_config.ft_attn_module](dim=dim, peft_config=peft_config)
        if peft_config.ft_mlp_module:
            self.ft_mlp_module = MODULE_REGISTRY[peft_config.ft_mlp_module](dim=dim, peft_config=peft_config)
    ...
    def forward(self, x: torch.Tensor, idx) -> torch.Tensor:
        ...
        # ft mlp module
        if self.peft_config.ft_mlp_module:
            if self.peft_config.ft_mlp_mode == 'parallel':
                x_original = self.drop_path2(self.ls2(self.mlp(x_norm2, idx)))
                if self.peft_config.ft_mlp_ln == 'before':
                    x_ft_mlp = self.drop_path2(self.ls2(self.ft_mlp_module(x))) + x_original
                elif self.peft_config.ft_mlp_ln == 'after':
                    x_ft_mlp = self.drop_path2(self.ls2(self.ft_mlp_module(x_norm2))) + x_original
                else:
                    raise NotImplementedError
                del x_original
            elif self.peft_config.ft_mlp_mode == 'sequential_after':
                x_original = self.drop_path2(self.ls2(self.mlp(x_norm2, idx)))
