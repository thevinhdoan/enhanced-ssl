...
def send_model_cuda(args, model, clip_batch=True):
    if not torch.cuda.is_available():
        ...
        assert False, "Distributed training is not supported yet"
...
class AverageMeter(object):
    ...
    def update(self, val, n=1):
        ...
        if self.count != 0:
            ...
            self.avg = 0
        # print(f"Updating {self.name} with val: {val} and n: {n}. Accumulated sum: {self.sum}, count: {self.count}, avg: {self.avg}")
