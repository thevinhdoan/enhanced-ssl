...
class BasicDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is None:
        ...
        elif self.alg == 'extract_pl':
            # return {'idx_ulb': idx, 'x_ulb': img_w, 'y_ulb': target}
            if isinstance(img, np.ndarray):
                img = Image.fromarray(img)
            img_w = self.transform(img)
            return {'idx': idx, 'x': img_w, 'y': target}
        else:
            if isinstance(img, np.ndarray):
                img = Image.fromarray(img)
            img_w = self.transform(img)
            if not self.is_ulb:
                ...
                # elif self.alg == 'sequencematch' or self.alg == 'somematch':
                    ...
                    x_ulb = self.val_transform(img)
                    return {'idx_ulb': idx, 'x_ulb': x_ulb, 'x_ulb_w': img_w, 'x_ulb_s': self.strong_transform(img), 'y_ulb': target}
                elif self.alg == 'finessl':
